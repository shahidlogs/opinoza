...
//
// computePreviewData: single batch query for all question IDs on the page.
// Returns a map of questionId → { rows: [{ label, count, pct }] }.
// No N+1: one SQL call regardless of page size.
//
// Preview is NOT cached with the feed because it changes with every new answer
// and the batch query is fast (indexed GROUP BY on answers table).

type PreviewRow = { label: string; count: number; pct: number };
type PreviewData = { rows: PreviewRow[] };
...
async function computePreviewData(
  ...
  const qTypeMap = new Map<number, string>(questions.map(q => [q.id as number, q.type as string]));

  // Single CASE expression: pick the right "group key" per answer row.
  // • poll   → poll_option text
  // • rating → rating as text ("4", "5", …)
  // • short  → normalized_answer if admin-set, else LOWER(TRIM(answer_text))
  ...
  // Group rows by question_id (Drizzle already sorted them for us per question).
  ...
  for (const [qid, entries] of grouped) {
    ...
    if (type === "rating") {
      ...
      // short_answer: show count, not percentage
...
async function attachPreviews(questions: any[]): Promise<any[]> {
...
router.get("/questions", async (req, res): Promise<void> => {
  ...
  if (cacheKey) {
    ...
    if (cached) {
      const questions = await attachPreviews(await applyUserContext(cached.questions));
  ...
  if (search?.trim()) {
    ...
    const questions = await attachPreviews(await applyUserContext(fetchResult));
  ...
  // Resolve isAnswered and attach live preview data for this page.
  const questions = await attachPreviews(await applyUserContext(fetchResult));
