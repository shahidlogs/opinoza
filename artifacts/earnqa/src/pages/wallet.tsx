...
export default function Wallet() {
  ...
  const MIN_WITHDRAWAL = 2000; // $20 in cents
          ...
          <div className="mb-8">
            ...
            {belowMinimum && balance > 0 && (
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800">Minimum withdrawal amount is $20</p>
                  <p className="text-xs text-amber-700 mt-0.5">
                    You have <span className="font-bold">{balance}¢</span> — need <span className="font-bold">{2000 - balance}¢</span> more to unlock withdrawals.
                  </p>
                </div>
              </motion.div>
            )}
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2">
                  Amount <span className="text-muted-foreground font-normal">(cents · min 2000¢ = $20)</span>
                </label>
                ...
                {amount && !isNaN(amountNum) && amountNum > 0 && amountNum < MIN_WITHDRAWAL && (
                  <p className="text-xs text-amber-700 mt-1.5 flex items-center gap-1 font-medium">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4m0 4h.01"/></svg>
                    Minimum withdrawal amount is $20 (2000¢)
                  </p>
                )}
                ...
                <div className="flex gap-2 mt-2">
                  {[2000, 5000, 10000, balance].filter((v, i, arr) => v > 0 && v <= balance && arr.indexOf(v) === i).slice(0, 4).map(preset => (
                    <button
                      key={preset}
                      type="button"
